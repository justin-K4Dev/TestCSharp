    ...
    static void RequiredMembers_what()
    {
        /*
            ...
            ❗ 문제점: required만 사용할 경우의 한계
               문제	                                        설명
            🔸 1. 값은 필수지만, 변경은 언제든 가능	        set 접근자는 초기화 이후에도 값을 바꿀 수 있습니다. 즉, 불변성이 보장되지 않습니다.
            🔸 2. 불변 객체와 궁합이 나쁨	                초기화 이후 값을 고정하려면 init이 필요하지만, set만 있으면 외부 수정 가능
            🔸 3. 초기화 후 실수로 값 덮어쓰기 가능	        생성 이후 의도치 않게 속성이 변경될 수 있는 여지가 있음
       */
        {
